...
export default class NursingPage {
  ...
  /**
   * @Test1 Verify Outpatient Checkout Process 
   *
   * @param {Record<string, string>} data - Not used in this method but typically used to pass additional parameters if needed.
   * @returns {Promise<void>} - Returns void; logs error if any step fails.
   *
   * Steps:
   * 1. Navigate to https://healthapp.yaksha.com/Home/Index#/Nursing/OutPatient
   * 2. Locate the same patient used in the previous test cases.
   * 3. Select the checkbox for that patient.
   * 4. Click on the Conclude button.
   * 5. Check out modal should opens.
   * 6. Click on Save button

   */

  async verifyOutpatientCheckoutProcess() {
    await this.page.waitForTimeout(2000);
    this.nursing_tab.click();
    await this.page.waitForTimeout(1000);
    this.checkbox.first().isChecked();
    await this.conclude.waitFor({ state: "visible" });
    this.conclude.click();
